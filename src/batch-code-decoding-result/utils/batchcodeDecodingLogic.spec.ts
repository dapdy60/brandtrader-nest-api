...
import { BatchCodeDecoderDto } from '../dto/batch-code-decoder-dto'; // Adjust this import path as necessary
  ...
  describe('decodeBatch', () => {
    it('decodes batch codes using specific brand decoders', async () => {
      const mockResult: DecodeResult = {
        ...
        month: 6,
        day: 9,
        output: 'Manufacturing Date: June 9, 2022',
        usedFallback: false,
      };
      mockDecodeParlux.mockResolvedValue(JSON.stringify(mockResult));

      const item: BatchCodeDecoderDto = {
        brand: 'billie-eilish',
        batchCode: '22160',
        brandId: '',
        productType: '',
      };
      await expect(batchCodeService.decodeBatch(item)).resolves.toEqual(
        mockResult,
      );
      ...
      const mockResults: DecodeResult[] = [
        {
          year: 2022,
          month: 6,
          output: 'Manufacturing Date: June, 2022',
          usedFallback: false,
        },
        {
          output: 'No valid decoder found for this batch code',
